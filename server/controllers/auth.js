...
exports.signup = async (req, res) => {
  try {
    ...
    if (userExist)
      return res
        .status(400)
        .json({ error: "User already registerd", status: "failed" });
    ...
    if (!result)
      return res
        .status(400)
        .json({ error: "Unable to registered user", status: "failed" });
    return res
      .status(200)
      .json({
        result: result,
...
exports.signin = async (req, res) => {
  try {
    ...
    if (!userExist)
      return res
        .status(400)
        .json({ error: "user doesnot exist", status: "failed" });
    ...
    if (!verifiedPassword)
      return res
        .status(400)
        .json({ error: "invalid credentials", status: "failed" });
